...
void	run_take(void)
{
	...
	takeable_items = retrieve_takeable_items(PLAYER->current_location->items,
		0);
...
static void	take_item(t_item *item_to_take)
{
	...
	{
		PLAYER->current_location->items[i]
			= PLAYER->current_location->items[index_of_last_location_item];
		memset((PLAYER->current_location->items + index_of_last_location_item),
			0, sizeof(t_item *));
	}
